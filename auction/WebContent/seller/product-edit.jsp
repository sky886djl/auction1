	...
	<form action="" method="post" class="form form-horizontal" id="form-product-add" enctype="multipart/form-data">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="hidden" value="${param.id }" name="proId" />
				<input type="text" class="input-text" value="${param.name }" placeholder="" id="proName" name="proName">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>当前价：</label>
			<div class="formControls col-xs-8 col-sm-9">
				${param.nowPrice }
				元</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" name="description" id="description" placeholder="" value="${param.description }" class="input-text" style="width:90%" />
				</div>
		</div>
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button id="btn" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe603;</i> 完成修改</button>
...
function remove(id){
	var type='删除一张图片';
	var url='../product/delPic';
	var delPic=document.getElementById('picShow'+id).src.split('product/')[1];
	var data={"id":"${param.id}","delPic":delPic};
	...
	$.ajax({
		...
		success: function(data){
			...
			for(var i=0;i<datas.length;i++){
				data=datas[i];
				if(data.typeName=='${param.typeName}'){
					category.append("<option selected='selected' value='"+data.id+"'>"+data.typeName+"</option>");
				}
				else {
...
$(document).ready(function(){
	...
	var pics=pic_url.split(";");
	for(var i=0;i<pics.length-1;i++){
		array1[i]=1;
		document.getElementById('picDiv'+(i+1)).style.display="block";
		document.getElementById('picShow'+(i+1)).src='../statics/images/product/'+pics[i];
		document.getElementById('clear'+(i+1)).style.display="block";
	}
	for(var i=0;i<3;i++){
		if(array1[i]==0){
			document.getElementById('picDiv'+(i+1)).style.display="block";
	...
	$('#startDate').val(formatDate1('${param.start_time }'));
	$('#endDate').val(formatDate1('${param.deadline }'));
});
